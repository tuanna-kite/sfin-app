...
import { LoanRequestForm, createLoan } from "../types/debt";
...
const LoanRequest = ({ navigation, route }: Props) => {
  ...
  async function updateLoanRequest() {
    try {
      ...
      await createLoan(user!.phone, { ...formData, loan: formData.loan * 1000000 });
      onSendRequest();
    } catch (error) {
      ...
      <Column px={5} flex={1} mb={12} justifyContent="space-between">
        <Box>
          ...
          <PrimaryInput
            ...
            onDoChange={onInputChange<LoanRequestForm>("reason", setFormData, formData)}
          />
          <PrimaryInput
            label="Thông tin tài khoản Momo"
            placeholder="Họ tên"
            autoCapitalize="words"
            onDoChange={onInputChange<LoanRequestForm>("fullname", setFormData, formData)}
          />
          <PrimaryInput
            placeholder="Số điện thoại Momo"
            keyboardType="number-pad"
            onDoChange={onInputChange<LoanRequestForm>("momo", setFormData, formData)}
          />
        </Box>
